...
- **Kuvaus:** Admin voi peruuttaa tai siirtää varauksia järjestelmässä. 
...
Järjestelmässä käytetään PostgreSQL-tietokantaa. Tietokanta tallentaa käyttäjien tiedot (mm. nimi, sähköpostiosoite, salasana) ja ajanvaraukset (päivämäärä, kellonaika, käyttäjä). Tietokannan rakenteet on suunniteltu siten, että ne tukevat tehokasta hakua ja varauksen hallintaa. Käyttäjien salasanat tallennetaan bcrypt-algoritmilla hashattuina, mikä takaa tietoturvan.
